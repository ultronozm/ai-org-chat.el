...
- =ai-org-chat-add-buffer-context= (=C-c a b=) :: This adds buffers as context directly to the node at point.
- =ai-org-chat-add-buffer-context= (=C-c a f=) :: This adds files as context directly to the node at point.
...
The commands =ai-org-chat-add-buffer-context= and =ai-org-chat-add-file-context= add "permanent" context, independent of which windows are visible, to the current node.  The context is specified by a list of files or buffers, contained in the :CONTEXT: property of the node.  Child nodes inherit the context of their parent nodes.  You can use a top-level property drawer to set context for an entire file.
