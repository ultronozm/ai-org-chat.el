...
You'll need to configure the =ai-org-chat-provider= variable with a valid provider as defined by the =llm= library.  Here's how to do that:
...
* Usage
When you want to ask the AI something, do =M-x ai-org-chat-new= (or =C-c /=, if you followed the above configuration).  This visits a new file in the specified directory ("~/gpt" by default).  If the region was active, then it will be quoted in the new buffer and a reference to the source will be stored in the =SOURCE_BUFFER= property.  This maintains a live connection to the original content so the AI always has access to the most recent version.  You can add additional source buffers by setting the =SOURCE_BUFFER= property in other conversation nodes.  With a prefix argument (=C-u=), it will immediately add visible buffers as [[Context][context]] to the new chat.  Example:
...
1. Select the region that you want to modify.
2. =C-x t 2= (=tab-bar-new-tab=) to create a new tab, then =C-c /= (or =M-x ai-org-chat-new=) to launch a chat session with the selected region.  The region's content will be quoted in the new buffer and a live connection to the source will be maintained via the =SOURCE_BUFFER= property.
...
4. When you receive a response in a source block, use =C-c e= (=ai-org-chat-compare=) to inspect what was changed, and standard Ediff commands to apply parts of that change.  The comparison will automatically use the source buffer referenced in the =SOURCE_BUFFER= property.
5. Iterate until you're happy with the changes.

An alternative workflow that doesn't rely on the =SOURCE_BUFFER= property:
